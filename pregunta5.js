...
function changeLabel(beers) {
  const newLabel = ("https://tecnoshare.sharepoint.com/sites/beer/[carpeta]/[nombre_archivo]")

  beers.forEach(beer => {
    beer.label = newLabel.replace("[carpeta]", beer.label.split("/")[5].toString()).replace("[nombre_archivo]", beer.name.split(" ").join("-") + ".png")
  }
  );
}
